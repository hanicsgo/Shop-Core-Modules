...
public Plugin myinfo =
{
	...
	author = "Xines and hani from anhemyenbai",
...
public void OnPluginStart()
{
	...
	//Configs
	iGroupID = CreateConVar("sm_groupid_add", "41691139", "Steam Group ID (Replace with yours)", FCVAR_NOTIFY);
...
void addcredits(int client)
{
	if(IsClientInGame(client) && b_IsMember[client]) //ingame + member
	{
		...
		//Give Credits
		Shop_SetClientCredits(client, Shop_GetClientCredits(client) + pcredits);
		
		//Print to client
		if(group_adverts.BoolValue && i_advert[client])
		{
			PrintToChat(client, "​\x04[SHOP]\x01 You received \x04%i\x01 credits for being member in our \x04steam group\x01!. Type \x04!shopee\x01 to use it!", pcredits);
		}
	}
}
